...
class BestellungSeite extends Page
{
    ...
    private function generatePizzaListView(array $pizzaArray):void
    {
        foreach($pizzaArray as $pizza){
            ...
            echo <<< EOT
    <figure>
      <img src="$pizzaSrc" alt="" width="100" height="100" onclick="addToCart($pizzaID, $pizzaName, $pizzaPrice)">
...
EOT;
    ...
    <article>
      ...
      <label>
        Ihre Adresse:
      <input type="text" id="adresse" name="adresse" value="" placeholder="Ihre Adresse" required>
      </label>
      <br>
      <label>E-Mail:
        ...
        if(count($_POST)){
            if($this->checkIfPostIsset()){
                ...
                foreach ($bestellungList as $bestellung){
                    ...
                    $bestellungName = $this->_database->real_escape_string($bestellung);
                    if($this->getPizzaID($bestellungName) != null){
                        $articleid = intval($this->getPizzaID($bestellungName));
                    }
                    else
                        return;
                ...
                header("Location: http://localhost/Praktikum/Prak3/KundeSeite.php");
                die();
            }
        }
    }
