...
EOT;
  ...
  <form action="BestellungSeite.php" id="BestellungInfos" method="post" lang="de" accept-charset="UTF-8">
    ...
    <div>
      <button type="button" onclick="deleteAllCartItems()">Alle Löschen</button>
      <button type="button" onclick="deleteSelectedCartItems()">Auswahl Löschen</button>
      <button type="submit" id="order-button" disabled="" onclick="sendData()">Bestellen</button>
    </div>
    </div>
  </form>
</div>
    ...
    protected function processReceivedData():void
    {
        ...
        if(count($_POST)){
            if($this->checkIfPostIsset()){
                ...
                header("Location: http://localhost/Praktikum/Prak5/KundeSeite.php");
                die();
            }
        }
    }
